...
let handler = async (m, { conn, args, usedPrefix, command }) => {
    if (!args[0]) throw `*Este comando descarga stickers de Telegram*\n\nEjemplo:\n${usedPrefix + command} https://t.me/addstickers/menggokil`
    if (!args[0].match(/(https:\/\/t.me\/addstickers\/)/gi)) throw `*Link incorrecto! Este comando descarga stickers de Telegram*\n\nEjemplo:\n${usedPrefix + command} https://t.me/addstickers/menggokil`
    ...
    m.reply(`*Total:* ${json.result.length}\n*Estimado completo:* ${json.result.length * 2} segundos\n\n` + mes)
    for (let i of json.result) {
        ...
        .catch(() => { conn.reply(m.chat, `_Error! Inténtalo nuevamente_`, m) })
        await delay(2000)
    }
}
