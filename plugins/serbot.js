...
let handler = async (m, { conn, args, usedPrefix, command, isOwner }) => {
  if (!global.db.data.settings[conn.user.jid].jadibot) return conn.sendButton(m.chat, 'serbot está inactivo', '', isOwner ? `Aktifkan` : `kevin`, isOwner ? `${usedPrefix}1 jadibot` : `${usedPrefix}owner`, m)
